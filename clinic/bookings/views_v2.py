...
from .models import Billing, Booking, Service, Patient, MedicalRecord, Inventory, Appointment, Payment
...
def htmx_medical_images(request, record_id):
    ...
    try:
        record = MedicalRecord.objects.prefetch_related('images').get(id=record_id)
        images = record.images.all()
        
        if not images:
            return HttpResponse(
                '<div class="alert alert-info">No medical images found for this record.</div>'
            )
...
def htmx_inventory_adjust(request, item_id):
    ...
    try:
        ...
        html = f'''
        <form hx-post="/v2/htmx/inventory-adjust/{item_id}/" hx-swap="outerHTML" hx-target="#inventory-adjust-modal-body">
            <div class="mb-3">
                <h5>{item.name}</h5>
            ...
            <div class="d-grid gap-2">
                ...
                <button type="button" class="btn btn-secondary" onclick="document.getElementById('inventoryAdjustModal').classList.remove('show')">
                    Cancel
                </button>
            </div>
        </form>
...
def htmx_inventory_adjust_submit(request, item_id):
    ...
    try:
        ...
        # Create stock transaction record (if model exists)
        ...
        return HttpResponse(
            ...
            <div class="alert alert-success">
                ...
                <br>New quantity: <strong>{item.quantity}</strong>
            </div>
            <script>
                setTimeout(() => {{
                    document.getElementById('inventoryAdjustModal').classList.remove('show');
                    htmx.trigger('#inventory-container', 'refreshInventory');
                }}, 1500);
            </script>
            '''
        )
...
def htmx_inventory_create(request):
    ...
    try:
        ...
        items = Inventory.objects.all().order_by('-date_stock_in')[:50]
        return render(request, 'bookings_v2/partials/inventory_list.html', {
            'items': items
        })
